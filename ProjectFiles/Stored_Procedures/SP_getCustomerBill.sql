...
DECLARE @date date;
	...
	WHILE @bID IS NOT NULL
		...
		SET @name = (SELECT top 1 name FROM customer WHERE telephone = (SELECT TOP 1 telephone FROM #tempBill))
		
		SET @address = (SELECT top 1 address FROM customer WHERE telephone = (SELECT TOP 1 telephone FROM #tempBill))
		DECLARE @path VARCHAR(MAX)
		SET @path = 'C:\Users\Autobots\Desktop\ProjectFiles\Bills\Bill.xlsx'
